import React, { useEffect, useContext } from 'react'
import GameSocketContext from '../contexts/gameSocket'
import Sketch from 'react-p5'

interface GameState {
  ballX: number;
  ballY: number;
  player1Y: number;
  player2Y: number;
  status: string;
}

function Board() {
  const PADLLE_WIDTH = 20;
  const PADLLE_HEIGHT = 100;
  const TABLE_W = 1000;
  const TABLE_H = 600;

  const socket = useContext(GameSocketContext);

  useEffect(() => {
    socket.emit('game_data', {
      gameId: window.location.pathname.split('/')[2],
    })
  }, [])

  var GameState: GameState = {
    ballX: TABLE_W / 2,
    ballY: TABLE_H / 2,
    player1Y: TABLE_H / 2 - PADLLE_HEIGHT / 2,
    player2Y: TABLE_H / 2 - PADLLE_HEIGHT / 2,
    status: 'waiting',
  }

  const setup = (p5: any, canvasParentRef: Element) => {
    p5.createCanvas(TABLE_W, TABLE_H).parent(canvasParentRef);
    p5.frameRate(60);
  };

  const drawPadlles = (p5: any) => {
    p5.fill(255);
    p5.rect(0, GameState.player1Y, 20, PADLLE_HEIGHT);
    p5.rect(TABLE_W - PADLLE_WIDTH, GameState.player2Y, 20, PADLLE_HEIGHT);
  };

  const drawMidLine = (p5: any) => {
    p5.stroke(255);
    p5.strokeWeight(4);
    p5.line(p5.width / 2, 0, p5.width / 2, p5.height);
  }

  const drawBall = (p5: any) => {
    p5.ellipseMode(p5.CENTER);
    p5.fill(255);
    p5.ellipse(GameState.ballX, GameState.ballY, 20);
  }

  const draw = (p5: any) => {
    p5.background(20, 12, 12);
    drawPadlles(p5);
    drawMidLine(p5);
    drawBall(p5);
    socket.emit('move_player', {
      gameId: window.location.pathname.split('/')[2],
      y: p5.mouseY,
    });
  };

  socket.on('game_state', (data: GameState) => {
    GameState = data;
  });

  return (<>
    <div className="w-full h-full">
      <Sketch
        setup={setup}
        draw={draw}
      />
    </div>
  </>
  )
}

export default Board